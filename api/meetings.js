// api/meetings.js — We Do Recover
// Live data only. AA/NA come from real public feeds at request time.
// All other pathways: community-submitted meetings (moderated) + official finder links.

const SUPA = "https://qdxufynzilmqzhortyrq.supabase.co";
const SUPA_KEY = "sb_publishable_DH4eF8AYI5K57iS9I3z4bQ_KvR2me39";

const AA_SOURCES = {
  nyc: { name: "New York Inter-Group", url: "https://www.nyintergroup.org/wp-admin/admin-ajax.php?action=meetings", homepage: "https://www.nyintergroup.org", center: { lat: 40.7484, lng: -73.9967 } },
...
async function getAA(lat, lng, radius) {
  let key = "nyc", best = Infinity;
  for (const [k, s] of Object.entries(AA_SOURCES)) {
    const d = miles(lat, lng, s.center.lat, s.center.lng);
    if (d < best) { best = d; key = k; }
  }
  const src = AA_SOURCES[key];
  const rows = await j(src.url);
  if (!Array.isArray(rows)) return { meetings: [], src };
  const meetings = rows
...
export default async function handler(req, res) {
  ...
  tasks.push(getCommunity(fellowship, la, ln, rad).then(m => out.meetings.push(...m)).catch(() => {}));
  tasks.push(getNotices(fellowship, la, ln, rad).then(n => { out.notices = n; }).catch(() => {}));
